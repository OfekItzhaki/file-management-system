      ...
      {isEditing ? (
        <div className='create-actions' style={{ flex: 1 }}>
          <input
            ...
            autoFocus
          />
          <button className='btn-primary' style={{ padding: '0.4rem 0.6rem' }} onClick={handleSave}>
            ✓
          </button>
          <button
            className='btn-secondary'
            style={{ padding: '0.4rem 0.6rem', background: '#ef4444' }}
        ...
        <>
          <span className='folder-name' onClick={() => onSelect(folder.id!)}>
            📁 {folder.name?.trim().toLowerCase() === 'default' && '🛡️ '}
            {folder.name}
